module.exports = async function handler(req, res) {
  ...
  if (!API_KEY) { res.status(500).json({ error: 'API key not set' }); return }
  ...
  try {
    ...
    if (!response.ok) { res.status(response.status).json({ error: data?.error?.message || 'Gemini error' }); return }
